...
import { useMobile } from '@/contexts/MobileContext'; // Use the MobileContext instead
...
export function SessionList({ sessions }: SessionListProps) {
  ...
  const renderCompletedValues = (values: ValueWithReason[]) => {
    ...
    if (isMobile) {
      return (
        <div 
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="mobile-values-title"
        >
          <div 
            className="bg-white rounded-lg p-4 max-w-sm w-full max-h-[80vh] overflow-y-auto"
            role="document"
          >
            <div className="flex justify-between items-center mb-4">
              ...
              <span 
                ...
                aria-label={`Total values: ${values.length}`}
              >
            ...
            <div 
              className="space-y-4"
              role="list"
              aria-label="Selected core values"
            >
              {values.map((value) => (
                <div 
                  key={value.id} 
            ...
            <button
              ...
              className="mt-4 w-full px-4 py-2 bg-blue-600 text-white rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
              aria-label="Close values view"
            >
              Close
            </button>
          </div>
    ...
    return (
      <section 
        className="space-y-4"
        aria-labelledby="desktop-values-title"
      >
        <div className="flex justify-between items-center">
          <h3 
            id="desktop-values-title"
            className="text-lg font-bold"
          >
        ...
        {sessions.map((session) => (
          ...
          >
            <div className="flex justify-between items-start mb-2">
              ...
              <div 
                className={`text-sm ${session.completed ? 'text-green-600' : 'text-blue-600'}`}
                role="status"
              >
            ...
            <div className="mt-3 flex justify-end">
              {session.completed ? (
                <button
                  ...
                  className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                  aria-label={`Show values for session from ${formatDate(session.timestamp)}`}
                >
                  Show Values
                </button>
              ) : (
                <Link
                  ...
                  className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                  aria-label={`Resume session from ${formatDate(session.timestamp)}`}
                >
                  Resume Game
                </Link>
              )}
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2" role="cell">
                {session.completed ? (
                  <>
                    <button
                      onClick={() => handleShowValues(session.id)}
                      className="px-3 py-1.5 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                      aria-label={`Show values for session from ${formatDate(session.timestamp)}`}
                    >
                      Show Values
                    </button>
                    {process.env.NODE_ENV === 'development' && (
                      <Link
                        ...
                        className="px-3 py-1.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                        aria-label={`Replay session from ${formatDate(session.timestamp)}`}
                      >
                        Replay
                      </Link>
                    )}
      ...
      <AnimatePresence>
        {showValuesFor && (
          <div 
            ...
            role="region"
            aria-label="Selected values details"
          >
