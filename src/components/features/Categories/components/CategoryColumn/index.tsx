...
export default function CategoryColumn({
  ...
  return (
    <div
      className="w-full rounded-lg bg-white p-4 shadow-sm transition-colors duration-200"
      data-category={title}
      aria-label={`${title} category`}
    >
      <div className="mb-4 flex items-center justify-between">
        <h2 className="flex w-full items-center justify-between text-lg font-semibold text-black">
          <span>{title}</span>
          <span className="ml-2 rounded-full bg-gray-100 px-2.5 py-0.5 text-sm font-medium text-black">
            {cards.length}
          </span>
        </h2>
      </div>
      ...
      <div
        className="min-h-[400px] rounded-lg border-2 border-dashed border-gray-400 p-2 transition-colors duration-200"
      ...
      >
          ...
          {cards.map((card, index) => (
            <div key={card.id} className="transition-all duration-200">
              <Card
                ...
                onDrop={(value) => onDrop(value, title, index)}
                onMoveUp={index > 0 ? () => onMoveWithinCategory(index, index - 1) : undefined}
                onMoveDown={index < cards.length ? () => onMoveWithinCategory(index, index + 1) : undefined}
        ...
        <div
          className={`mt-2 py-4 text-center text-sm text-black ${cards.length === 0 ? '' : 'border-t-2 border-dashed border-gray-400'}`}
        >
          Drop values here
        </div>
      </div>
