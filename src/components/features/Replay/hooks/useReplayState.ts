...
export function useReplayState() {
  ...
  const executeCommand = useCallback(
    (command: Command, availableCards?: Value[]) => {
      if (availableCards && allCards.length === 0) {
        setAllCards(availableCards);
      }

      const { cardId } =
        command.type === 'DROP' ? (command.payload as DropCommandPayload) : (command.payload as MoveCommandPayload);
      ...
      if (command.type === 'DROP') {
        ...
        setCategories((prev) => {
          ...
          if (fromCategory === toCategory) {
            ...
            newCategories[fromCategory] = (prev[fromCategory] || []).filter((c) => c.id !== cardId);
