...
export const handleShowValues = async (
  sessionId: string,
  setIsLoading: (loading: boolean) => void,
  setCurrentValues: (values: Value[]) => void,
  setShowValuesFor: (sessionId: string | null) => void
) => {
  setIsLoading(true);
  try {
...
export const handleImportSession = async (
  ...
  if (file) {
    ...
    reader.onload = async (e) => {
      try {
        ...
        let { session, completedSession, rounds } = importData as {
          session: Session;
