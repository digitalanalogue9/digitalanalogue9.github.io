...
import { logStateUpdate } from '@/utils/debug/renderLogger';
...
export function useMobileInteractions() {
  ...
  const handleExpand = useCallback((category: CategoryName) => {
    logStateUpdate('handleMobileExpand', { category }, 'MobileInteractions');
  ...
  const handleDropWithZone = useCallback((card: Value, category: CategoryName) => {
    logStateUpdate('handleMobileDropWithZone', { card, category }, 'MobileInteractions');
